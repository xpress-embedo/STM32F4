...
int main(void)
{
  // 1) Get the address of RCC Clock Configuration Register
  uint32_t *p_rcc_cfg_reg = (uint32_t*)(RCC_CFGR_ADDR);

  // 2) Configure the RCC Clock Configuration Register MCO1 bit fields to select
  // the HSI as Clock Source (HSI is internal and by default clock source of the
  // controller and for this controller it's value is 16 MHz)
  ...
  // Since we want to output HSI clock on MCO1, we will do the following
  ...
  *p_rcc_cfg_reg |= (0x01 << 26u);
  ...
  // 3) MCO1 signal is internal to controller, and to see this signal we need to
